...
<div class="card">
    ...
    <div class="card-header border-0 pt-6">
        <h4 class="mb-sm-0 font-size-18">Update Cart</h4>
    ...
    <div class="card-body pt-0">
        <!--begin::Table-->
        <form method="post" action="{{ route('cart.update', $row->cart_id) }}">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" name="customer_number" data-control="select2" required data-placeholder=" Pilih Customer " data-allow-clear="true">
                    ...
                    @foreach ($customer as $cus)
                        <option value="{{ $cus['customer_number'] }}" {{ ( $row->customer_number == $cus['customer_number'])? 'selected' : ''; }}>{{ $cus['customer_fullname'].' - '.$cus['customer_number'] }}</option> 
                    @endforeach  
                </select>
                <!--begin::Select2-->
                </div>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select" name="product_code" data-control="select2" required data-placeholder=" Pilih Product " data-allow-clear="true">
                    ...
                    @foreach ($product as $p)
                        <option value="{{ $p['product_code'] }}"  {{ ($row->product_code == $p['product_code'])? 'selected' : ''; }}>{{ $p['product_name'].' - Rp. '.$p['product_price_purchase'] }}</option> 
                    @endforeach  
                </select>
                <!--begin::Select2-->
                </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input name="cart_quantity" type="number" class="form-control" min="1" id="formrow-inputCity" required value="{{ $row->cart_quantity }}" placeholder="quantity">
                </div>
            </div>
            <div class="col-md-6">
                <div class="mb-3">
                    <label for="formrow-inputCity" class="form-label">Notes</label>
                    <textarea name="cart_note" type="text" class="form-control" id="formrow-inputCity" rows="2" required placeholder="seller note">{{ $row->cart_note }} </textarea>
                </div>
            </div>
        </div>
